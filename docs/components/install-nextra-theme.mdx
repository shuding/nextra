To support default search functionality, install **Pagefind** as a `devDependency`:
...
Add `_pagefind/` to `.gitignore`. This prevents committing the generated search index files, which are not necessary for version control.
