...
That’s where `_meta.js` comes in. You can have an `_meta.js` file in each
...
Alternatively, you can do it with `title` and have other configurations in there as well:
...
<Callout type="warning">
  If you are using [next-sitemap](https://github.com/iamvishnusankar/next-sitemap),
  you will probably need to add the `['*/_meta']` exclude to your `next-sitemap.config.js`
