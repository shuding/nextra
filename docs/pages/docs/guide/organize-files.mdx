...
It's possible to use the `.jsx`, `.ts` and `.tsx` extensions for `_meta` files as
well (e.g. `_meta.ts`).
...
[numbers are always ordered first](https://dev.to/frehner/the-order-of-js-object-keys-458d) for JavaScript objects.
