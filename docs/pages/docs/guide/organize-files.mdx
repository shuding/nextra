...
And the global page map will be imported to each page by Nextra. Then, configured
...
<Callout>
  It's also possible to use the `.jsx`, `.ts`, `.tsx` extensions for `_meta` files
  (e.g. `_meta.ts`)
</Callout>
...
<Callout>
  You can use ESLint's built-in rule sort keys with a `/*
  eslint sort-keys: error */` comment to sort your sidebar items alphabetically.
</Callout>
