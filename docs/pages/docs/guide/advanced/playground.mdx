...
For more information, see \`compileMdxClient.ts\`.`)
  return (
    <div className='grid grid-cols-2 gap-2 mt-6'>
      <textarea value={rawMdx} onChange={event => setRawMdx(event.target.value)} className={
        cn(
      ...
      <div>
        <Playground
          fallback={<div className='flex h-full items-center text-center text-4xl'>Loading playground component...</div>}
          source={rawMdx} />
      </div>
    </div>
 )
