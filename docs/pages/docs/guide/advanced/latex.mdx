...
Nextra can use [KaTeX](https://katex.org) to pre-render LaTeX expressions directly in MDX or [MathJax](https://www.mathjax.org/) to
...
supported [KaTeX options](https://katex.org/docs/options.html) via the `options` key in
...
When MathJax is enabled (by setting `latex: { renderer: 'mathjax' }`) math is rendered on page load via
