...
[KaTeX’s documentation](https://katex.org/docs/supported).
...
You can provide additional options, that will be passed to
[\`<MathJaxContext />\` component](https://github.com/fast-reflexes/better-react-mathjax#usage) as
\`props\`.

For example macros \`$\\RR$\` will be rendered as $\\RR$, while providing \`config\`:`
