...
[![Livepeer preview](./_logos/livepeer.png)](https://docs.livepeer.org/guides/overview)
...
[![KitQL preview](./_logos/kitql.png)](https://kitql.dev)
  ...
  return (
    <Cards className="[&_img]:object-cover [&_img]:[aspect-ratio:12/6.3]">
      {cloneElement(props.children, {
        components: {
          a(props) {
            const { alt } = props.children.props
            return (
              <Card href={props.href} title={alt.replace(' preview', '')}>
                {props.children}
              </Card>
            )
          }
