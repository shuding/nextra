...
import { createTypeTable, generateTsFromZod } from '@nextra/tsdoc'
import { ToggleVisibilitySection } from 'components/toggle-visibility-section'
import { NavbarPropsSchema } from 'private-next-root-dir/../packages/nextra-theme-docs/src/components/navbar'

# `<Navbar>` Component

|               |                          |                                         |                                                                                        |
| ------------- | ------------------------ | --------------------------------------- | -------------------------------------------------------------------------------------- |
| `logo`        | `ReactNode{:ts}`         | `'Nextra - The Next Docs Builder'{:ts}` | Logo of the website.                                                                   |
| `logoLink`    | `boolean \| string{:ts}` | `true{:ts}`                             | Specifies whether the logo should have a link or provides the URL for the logo's link. |
| `projectLink` | `string{:ts}`            |                                         | URL of the project homepage.                                                           |
| `projectIcon` | `ReactNode{:ts}`         | `<GitHubIcon height="24" />{:ts}`       | Icon of the project link.                                                              |
...
<TSDoc
  code={`type $ = ${generateTsFromZod(NavbarPropsSchema)}
export default $`}
/>
