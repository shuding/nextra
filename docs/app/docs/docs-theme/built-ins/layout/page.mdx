...
import { LayoutPropsSchema } from 'private-next-root-dir/../packages/nextra-theme-docs/src/layout'

export const layoutProps = generateTsFromZod(LayoutPropsSchema)
...
export function PartialTSDoc({ flattened, props }) {
  return (
    <TSDoc
      flattened={flattened}
      code={`
type $ = Pick<${layoutProps}, ${props.map(prop => JSON.stringify(prop)).join('|')}>
export default $`}
    />
  )
}
