...
You can configure the theme for each page using the `'theme'` option.

|               |                               |                                                                                 |
| ------------- | ----------------------------- | ------------------------------------------------------------------------------- |
| `breadcrumb`  | `boolean{:js}`                | Show or hide breadcrumb navigation. Default: `true`.                            |
| `collapsed`   | `boolean{:js}`                | Indicates whether the item in sidebar is expanded by default. Default: `false`. |
| `footer`      | `boolean{:js}`                | Specifies whether to display the footer. Default: `true`.                       |
| `layout`      | `'default' \| 'full'{:js}`    | Defines the layout style. Default: `'default'{:js}`.                            |
| `navbar`      | `boolean{:js}`                | Specifies whether to display the navbar. Default: `true`.                       |
| `pagination`  | `boolean{:js}`                | Determines if pagination controls are shown. Default: `true`.                   |
| `sidebar`     | `boolean{:js}`                | Specifies whether to display the sidebar. Default: `true`.                      |
| `timestamp`   | `boolean{:js}`                | Indicates if "last updated" timestamps are displayed. Default: `true`.          |
| `toc`         | `boolean{:js}`                | Determines whether a table of contents is displayed. Default: `true`.           |
| `typesetting` | `'default' \| 'article'{:js}` | Configures the text typesetting style. Default: `'default'{:js}`.               |

For example, you can disable or enable specific components for specific pages:
