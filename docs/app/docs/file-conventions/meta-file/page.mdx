...
In Nextra you can organizes files as follows:

- **Inside Next.js'
  [`app` directory](https://nextjs.org/docs/app/getting-started/project-structure#top-level-folders):**
  Nextra collects all your `page` files, including
  [`page.md` and `page.mdx` files](/docs/file-conventions/page-file) and
  [`_meta` files](/docs/file-conventions/meta-file).
- **Inside Nextra's
  [`content` directory](/docs/file-conventions/content-directory):** Nextra
  collects all your `.md` and `.mdx` files along with
  [`_meta` files](/docs/file-conventions/meta-file).

> [!TIP]
>
> You can also combine both organization structures in your project and have `content` directory with `.mdx` files and `app` directory with `page.mdx` files

Afterward, Nextra generates the page map information called `pageMap` for your
entire site. The `pageMap` contains all routes and the directory structure, enabling features like the navigation bar and sidebar shown below:
...
Below the same
file-based routing structure is represented for `content` and `app`-only
directories:
