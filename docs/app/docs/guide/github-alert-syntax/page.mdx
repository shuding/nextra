...
`nextra/components`. You should then create the `<Blockquote>` component by invoking the
`withGitHubAlert` function.

The first argument should be your standard
`<blockquote>` component, and the second argument should be a React HOC component
that handles the GitHub alert syntax.
...
To make the `<Blockquote>` component available, you can integrate it into the `useMDXComponents` function:
