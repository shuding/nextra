...
Migrate your Pages Router site with minimal changes using this mode. Follow
these steps:
...
Add `[[...mdxPath]]/page.jsx` file with the following content

export async function MDXPathPage() {
  const filename = '[[...mdxPath]]/page.jsx'
  const rawMdx = `\`\`\`mdx filename="${filename}"
${(await fs.readFile(`../examples/docs/src/app/docs/${filename}`, 'utf-8'))
  .split('\n')
  .slice(2, -1)
  .join('\n')}
\`\`\``
...
>   [static images](https://nextra.site/docs/guide/image#static-image) works as
>   well
>
> Checkout Nextra's
