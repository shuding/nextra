...
The `<TSDoc>` component from `nextra/tsdoc` generates a properties table from TypeScript
`type` and `interface` that displays:
...
> [!TIP]
>
> Description is parsed from [TSDoc](https://tsdoc.org) inline description or the `@description` tag.
> You can use any Markdown/MDX syntax here.
