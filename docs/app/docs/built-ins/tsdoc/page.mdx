...
A component for generating documentation from `type`, `interface`, and `function` definitions using
[TSDoc](https://tsdoc.org) annotations.
...
In this example, while the actual type of the property `foo` is `string`, the displayed type will be
`"not dimaMachina"` as per the `@remarks` tag.
