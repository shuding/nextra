...
describe('create-turbo cli', () => {
  ...
  describe('the --version flag', () => {
    it('prints the current version', async () => {
      const { stdout } = await execFile('node', [createTurbo, '--version'])
  ...
  describe('the -v flag', () => {
    it('prints the current version', async () => {
      const { stdout } = await execFile('node', [createTurbo, '-v'])
      expect(!!semver.valid(stdout.trim())).toBe(true)
    })
  })
  ...
  describe('the --help flag', () => {
    it('prints help info', async () => {
      const { stdout } = await execFile('node', [createTurbo, '--help'])
  ...
  describe('the -h flag', () => {
    it('prints help info', async () => {
      const { stdout } = await execFile('node', [createTurbo, '-h'])
      expect(stdout).toMatchInlineSnapshot(`
        "
          Create a new Turborepo
