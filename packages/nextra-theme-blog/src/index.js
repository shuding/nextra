  ...
  return (
    <React.Fragment>
      ...
      <article className="container prose prose-sm md:prose">
        {titleNode}
        {type === 'post' ? <Meta {...meta} back={back} config={config} /> : <Nav navPages={navPages} />}
        <MDXTheme>
          {contentNodes}
...
export default (opts, _config) => {
  const config = Object.assign({
    readMore: 'Read More →',
    footer: <small style={{ display: 'block', marginTop: '8rem' }}>CC BY-NC 4.0 2020 © Shu Ding.</small>,
    postFooter: null
  }, _config)
  ...
  return props => {
    ...
    const postList = posts ? <ul>{
      posts.map(post => {
        ...
        const postDate = post.frontMatter ? <time className="post-item-date">{(new Date(post.frontMatter.date)).toDateString()}</time> : null
        const postDescription = post.frontMatter && post.frontMatter.description
          ? <p className="post-item-desc">{post.frontMatter.description}{config.readMore ? <Link href={post.route}><a className="post-item-more">{config.readMore}</a></Link> : null}</p>
          : null

        return <div key={post.route} className="post-item">
          <h3><Link href={post.route}><a className="post-item-title">{postTitle}</a></Link></h3>
          {postDescription}
          {postDate}
        </div>
      })
