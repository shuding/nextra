...
import { RemoteContent, Callout } from 'nextra/components'
...
<Callout emoji="✅">
  **MDX** (the library), at its core, transforms MDX (the syntax) to JSX. It
  ...
  const { result } = await compileMdx(`~~~mdx filename="example.mdx"
${mdx}
~~~
...
${mdx}`)
  return <RemoteContent compiledSource={result} components={{ Callout }} />
}
